...
#age_at_first_birth -> y (the age when he has a child)
...
n = length(cleaned_gss_data$age_at_first_birth)
N = length(gss_data$age_at_first_birth)
fpc.srs = rep(N, n)

age.design <- svydesign(id=~1, data=cleaned_gss_data, fpc=fpc.srs)

mysvylm <- svyglm(age_at_first_birth ~ age_at_first_marriage, age.design)
summary(mysvylm)


# str(cleaned_gss_data)
...
The selected is a simple linear regression model $Y=\beta_0 + \beta_1x + e$ .Here, Y represents the variable like rate, x represents age. $\beta_0$ is the intercept parameter, $\beta_1$ is the slope parameter, e is the random error term.

We use a simple linear regression  $y= b_0 + b_1x$  to fit it. 
