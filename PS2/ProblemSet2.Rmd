...
#get the no NA "age_first_child" caseid
cleaned_gss_data = gss_data %>% 
  filter(!is.na(age_first_child)) %>%  
  mutate(age_when_having_first_child = age - age_first_child) %>% 
  select(age_when_having_first_child, age_at_first_marriage,education)

str(cleaned_gss_data)
my_LRmodel <- lm(age_when_having_first_child ~ age_at_first_marriage +
                   as.factor(education), data=cleaned_gss_data)
summary(my_LRmodel)

hist(cleaned_gss_data$age_when_having_first_child)
